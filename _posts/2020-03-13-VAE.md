...
这一方式对于复杂的数据集并不可行，我们可能需要采样大量的$z$才能对数据进行准确的估计。那么对于给定数据X，我们可以采样出最可能产生X的z吗？在VAE中，这个采样的概率被表达为Q(z|X)。对于Q(z|X)，我们希望其与P(z|X)尽量接近。另一方面，我们希望P(X)尽量大，即生成的数据更符合真实数据。VAE的损失函数可以从Q(z)和P(z|X)和的KL散度中推导出来。
